...
# Detecting defects
def detect_defects(autoencoder, defect_descriptors, threshold, type='ae'):
    """
    ...
    with torch.no_grad():
        ...
        if type == 'ae':
            ...
            reconstructions, _, _, _ = autoencoder(defect_descriptors)
            # Calculate the reconstruction error
            reconstruction_error = torch.mean((reconstructions - defect_descriptors) ** 2, dim=1)
...
# Detecting defects
def compute_recon_error(autoencoder, defect_descriptors, type='ae'):
    """
    Compute the reconstruction error from the autoencoder on descriptor matrix
    ...
    print(f'The recon. error has been calculate for the {len(defect_descriptors)} atoms')
    return reconstruction_error

def defect_indices_atoms(reconstruction_error,threshold):
    """
    Return the atoms indices for recon > threshold
